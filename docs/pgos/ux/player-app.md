...
|------|------|
| **画面ID** | P-HOME |
| **要件ID** | A1-1-01, A1-2-01, A1-3-01 |
| **目的** | アプリの起点。近くの店舗とイベント情報を一覧表示 |
...
**アクション:**
- 店舗カード → P-STORE-DETAIL
- イベントカード → P-EVENT-DETAIL
- 「すべて見る」→ P-STORE-LIST / P-EVENT-LIST
- マップサムネイル → P-SEARCH (マップモード)

**状態バリエーション:**
- 位置情報未許可: 位置情報許可を促すバナー表示
- お気に入り未登録: お気に入り登録を促すカード表示
- チェックイン中: 上部に「プレイ中: 店舗A」バナー表示
...
|------|------|
| **画面ID** | P-SEARCH |
| **要件ID** | A1-1-01〜A1-1-06, A1-2-01〜A1-2-05 |
| **目的** | マップまたはリストで店舗・イベントを検索 |
...
- タグ例: `#NLH` `#PLO` `#トーナメント` `#リングゲーム` `#初心者歓迎` `#ディープスタック` `#¥3,000以下` `#今日開催` `#空席あり`
- タグをタップするとフィルタ適用、再タップで解除
- 詳細フィルタ（ドロップダウン）との併用可能

**アクション:**
- マップピン / リスト項目 → P-STORE-DETAIL
- 「イベント」タブ切替 → イベントカレンダー/リスト表示
...
|------|------|
| **画面ID** | P-STORE-DETAIL |
| **要件ID** | A1-1-04, A1-2-04, A2-1-01 |
| **目的** | 店舗の詳細情報、現在のテーブル状況、今後のイベントを表示 |
...
|------|------|
| **画面ID** | P-EVENT-LIST |
| **要件ID** | A1-2-01, A1-2-02 |
| **目的** | 日付・条件でフィルタしたイベント一覧 |
